...
exports.footeradd=function (req, res) {
		...
		let footerObj = urlModule.parse(req.url, true).query;//解析数据 获得Json对象
		var _footer;
		let value = footerObj._title;//通过参数名称获得参数值
		_footer = new Footer({
			_id:footerObj._id,
...
exports.det= function (req, res) {
	...
	Footer.update({_id:id},{id:id,_title:footerObj._title,_copyright:footerObj._copyright,_record:footerObj._record}, function(error){
		if(error){
			res.json({
				data: 'fail'
			})
