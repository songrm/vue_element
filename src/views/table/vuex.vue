  ...
  <div>
    ...
    <el-form :model="formInline" class="demo-form-inline">
      <el-form-item label="审批人">
        <el-input v-model="formInline.user" placeholder="审批人" @keyup.enter.native="search_Filter"></el-input>
      </el-form-item>
    </el-form>
    ...
    <el-table
     :data="tableData6" :row-class-name="tableRowClassName"
     :span-method="objectSpanMethod"
     border
     style="width: 100%; margin-top: 20px">
     <el-table-column type="selection" width="55" class="selection" :selectable='checkboxInit' @selection-change="changeFun"></el-table-column>
     <el-table-column
       prop="id"
       label="ID"
       width="180">
    ...
    computed:{
     ...
     methods:{
       ...
       sort(){
         ...
         var sortable = Sortable.create(el,{
           onEnd: function(evt) {
             ...
             var obj=new Object()
             obj.age=num
             obj.name=$(itemEl).find('span').eq(1).text()
      ...
      abc01(){
        ...
        obj.age=''
        obj.name=''
        console.log($('#items li').eq(0).find('span').eq(0).text())
        this.newarr=[]
        for (var i = 0; i < 3; i++) {
          console.log(i)
          ...
          this.newarr.push({'name':$('#items li').eq(i).find('span').eq(0).text(),'age':$('#items li').eq(i).find('span').eq(1).text()})
        }
        for (var i = 0; i < this.newarr.length; i++) {
          if(this.newarr[i].name==this.arr[i].name){

          }else {
            this.qx.push({'name':this.newarr[i].name,'age':this.newarr[i].name,'sort':i})
          }
        }
        console.log(this.qx)
...
.el-table__row:hover,.el-table__row:hover td{background-color: rgba(0,0,0,0)!important}
